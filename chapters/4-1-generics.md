...
The model takes the generic [[K has F]] to mean the prevalence of property F within kind K --- i.e., P(F | K) --- is above some threshold. This threshold --- `theta` --- is thought to be in general unknown (`theta = uniform(0,1)`) and must be inferred in context. 
