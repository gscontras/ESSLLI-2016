...
With this utility function in mind, $$S_{1}$$ computes the probability of an utterance *u* given some state *s* in proportion to the speaker’s utility function $$U_{S_{1}}$$. The term *α > 0* controls the speaker’s optimality, that is, the speaker’s rationality in choosing utterances. (*α* corresponds to the temperature parameter of $$S_{1}$$’s soft-max optimization.)
